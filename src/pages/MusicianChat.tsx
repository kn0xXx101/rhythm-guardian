...
import MusicianContactDetail from '@/components/chat/MusicianContactDetail';
...
interface MusicianContact extends Contact {
  eventType: string;
}
...
const MusicianChat = () => {
  ...
  const { user } = useAuth();
  const [contacts, setContacts] = useState<MusicianContact[]>([]);
  const [selectedContact, setSelectedContact] = useState<MusicianContact | null>(null);
      ...
      return prev.map((contact) => {
        if (contact.id === AI_ASSISTANT_ID) {
          return {
            ...
            eventType: contact.eventType || 'Support',
          } as MusicianContact;
        }
        return contact;
      });
  ...
  // Preload hirer contact when opened via URL param (e.g., from booking)
  useEffect(() => {
    const userIdParam = searchParams.get('user') || searchParams.get('contact');
    if (!userIdParam || !user) return;

    const loadHirerContact = async () => {
      try {
        const { data: profile, error } = await supabase
          .from('profiles')
          .select('user_id, full_name, avatar_url, last_active_at')
        ...
        if (!paidBooking) {
          toast({
            ...
            description:
              'Chat opens after the hirer completes payment on an accepted booking.',
          });
          setSearchParams({});
          return;
        }
        ...
        const hirerContact: MusicianContact = {
          id: userIdParam,
          name: profile.full_name || 'Unknown Hirer',
        ...
        setContacts((prev) => {
          const exists = prev.find((c) => c.id === hirerContact.id);
          if (exists) return prev;
          return [...prev, hirerContact];
        });
        ...
        console.error('Error loading hirer contact:', error);
      }
    };
    ...
    const fetchContacts = async () => {
      ...
      setError(null);
      try {
        console.log('[MusicianChat] Starting fetchContacts for user:', user.id);
        ...
        console.log('[MusicianChat] Total messages found:', allMessages?.length);
        ...
        console.log('[MusicianChat] Unique message partners:', partnerIds);
        ...
        console.log('[MusicianChat] Profiles fetched:', profiles?.length);

        const uniqueContacts = new Map<string, MusicianContact>();
        ...
        partnerIds.forEach((partnerId) => {
          ...
          if (profile) {
            ...
            uniqueContacts.set(partnerId, {
              ...
              isOnline,
              lastSeen: profile.last_active_at || new Date().toISOString(),
              eventType: isAdmin ? 'Admin Support' : 'Event',
              publicKey: '',
        ...
        // 3. Add hirers from bookings who haven't messaged yet
        const { data: bookings, error: bookingsError } = await supabase
          .from('bookings')
          .select('hirer_id, profiles!bookings_hirer_id_fkey(user_id, full_name, avatar_url, last_active_at, role)')
          .eq('musician_id', user.id)
          .eq('payment_status', 'paid')
          ...
          bookings.forEach((booking: any) => {
            ...
            if (profile && !uniqueContacts.has(profile.user_id)) {
              ...
              uniqueContacts.set(profile.user_id, {
                id: profile.user_id,
                name: isAdmin ? 'Admin' : (profile.full_name || 'Unknown Hirer'),
                image: profile.avatar_url || '/placeholder.svg',
        ...
        const aiAssistantContact: MusicianContact = {
          id: AI_ASSISTANT_ID,
          name: 'AI Assistant',
          image: '/placeholder.svg',
          lastMessage: aiAssistantLastMessage.substring(0, 50),
        ...
        console.error('[MusicianChat] Error in fetchContacts:', error);
  ...
  useEffect(() => {
    if (!chatSettings?.privacy?.onlineStatus || contacts.length === 0) return;

    let isMounted = true;

    const refreshContactStatuses = async () => {
      const contactIds = contacts.map((contact) => contact.id?.toString()).filter(Boolean);
  ...
  }, [contacts, chatSettings?.privacy?.onlineStatus, updateContactStatus]);

  // Sync selectedContact with activeContactId from context
  useEffect(() => {
    if (activeContactId && contacts.length > 0) {
      const contact = contacts.find((c) => c.id === activeContactId);
      if (contact && (!selectedContact || selectedContact.id !== activeContactId)) {
        setSelectedContact(contact);
      }
    }
  }, [activeContactId, contacts, selectedContact]);
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (otherUserId === AI_ASSISTANT_ID) return;

          // Update contacts list
          setContacts((prev) => {
            const existingContactIndex = prev.findIndex((c) => c.id === otherUserId);
            ...
            if (existingContactIndex !== -1) {
              ...
              updated[existingContactIndex] = {
                ...
                unread: selectedContact?.id !== otherUserId,
                eventType: existingContact.eventType || 'Event',
              } as MusicianContact;
              // Move the updated contact to the beginning
              if (existingContactIndex > 0) {
                const moved = updated.splice(existingContactIndex, 1)[0];
            ...
            supabase
              ...
              .then(({ data: profile, error }) => {
                ...
                const newContact: MusicianContact = {
                  id: otherUserId,
                  name: isAdmin ? 'Admin' : (profile?.full_name || 'Unknown User'),
                  image: profile?.avatar_url || '/placeholder.svg',
                  lastMessage: msg.content?.substring(0, 50) || '',
                ...
                setContacts((current) => {
                  ...
                  if (current.find((c) => c.id === otherUserId)) return current;
  ...
  const handleSelectContact = (contact: Contact) => {
    const musicianContact = contact as MusicianContact;
    setSelectedContact(musicianContact);
    setActiveContactId(musicianContact.id);

    // Mark as read when selecting
    setContacts((prev) =>
      prev.map((c) => (c.id === musicianContact.id ? { ...c, unread: false } : c))
    );
  ...
  const handleSearchSelect = async (result: SearchContact) => {
    if (result.role === 'hirer' && user?.id && result.id !== AI_ASSISTANT_ID) {
      const { data: paidBooking } = await supabase
        .from('bookings')
        .select('id')
        .eq('musician_id', user.id)
        .eq('hirer_id', result.id)
        .eq('payment_status', 'paid')
        .limit(1)
        .maybeSingle();
      ...
      if (!paidBooking) {
        toast({
          ...
          title: 'Payment pending',
          description:
            'You can message this hirer after they pay for an accepted booking.',
        });
        return;
      }
    }
    ...
    const newContact: MusicianContact = {
      ...
      eventType: result.role === 'hirer' ? 'Hirer' : result.role,
      publicKey: '',
    };
  ...
  const renderContactDetail = (contact: Contact) => {
    ...
    return (
      <div className="space-y-2">
        <MusicianContactDetail eventType={musicianContact.eventType ?? 'Event'} />
  ...
  const renderHeaderDetail = () => {
    ...
    return (
      <div className="flex items-center gap-2">
        <MusicianContactDetail eventType={selectedContact.eventType} />
  ...
  return (
    <ChatLayout
      title={
        <DashboardHeader
          heading="Messages"
          text="Chat with hirers and manage your gig communications."
          className="mb-0"
        >
          <div className="flex items-center gap-2">
      ...
      contactsList={
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full p-6 text-center">
            <p className="text-destructive font-semibold">Error loading contacts</p>
            <p className="text-sm text-muted-foreground mt-2">{error}</p>
          </div>
        ) : contacts.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full p-6 text-center">
            <p className="text-muted-foreground">No active bookings yet</p>
            <p className="text-sm text-muted-foreground mt-2">Search for a hirer above to start chatting</p>
          </div>
        ) : (
          <ContactsList
            contacts={contacts}
