...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const enabled = metadata?.two_factor_enabled === true;

    if (!enabled || !secret) {
      return new Response(
        JSON.stringify({ success: false, requiresBackupCode: false }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    let success = false;
    let requiresBackupCode = false;
    let updatedBackupHashes = backupHashes;

    const isNumericCode = /^[0-9]{6}$/.test(code);

    if (isNumericCode) {
      success = authenticator.verify({ token: code, secret });
    }

    if (!success && backupHashes.length > 0) {
      const hash = await hashString(code);
      if (backupHashes.includes(hash)) {
        success = true;
        updatedBackupHashes = backupHashes.filter((value) => value !== hash);
      } else {
        requiresBackupCode = !isNumericCode;
      }
    }

    if (success && updatedBackupHashes !== backupHashes) {
      const updateResult = await supabaseClient.auth.updateUser({
        data: {
          two_factor_backup_codes: updatedBackupHashes,
        },
      });
    ...
    return new Response(
      JSON.stringify({ success, requiresBackupCode }),
      {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      },
