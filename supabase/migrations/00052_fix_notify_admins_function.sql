-- Fix notify_admins function parameter name conflict
-- Drop and recreate the function to avoid parameter name conflicts

-- Drop the existing function with all possible signatures
DROP FUNCTION IF EXISTS notify_admins(text,text,text,text,jsonb);
DROP FUNCTION IF EXISTS notify_admins(text,text,text,text);
DROP FUNCTION IF EXISTS notify_admins;
